...
@Service()
export class NoFluffAdScraper extends BaseAdScraper implements IJobAdScraper {
    /**
   * @description Function that accepts client form, and based on it starts the jobAd scraping.
   * Data available on NoFluff in the scrape is (jobTitle, companyName, companyLocation, salary, skills).
   ...
   */
    public async scrape(clientForm: ScrapeJobAdsForm): Promise<JobAdDTO[]> {
        ...
        return await this.scrapeAds(urlParams, JobAdSource.NO_FLUFF_JOBS);
    }
}
