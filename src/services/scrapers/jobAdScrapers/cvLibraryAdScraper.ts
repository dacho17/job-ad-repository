...
@Service()
export class CvLibraryAdScraper extends BaseAdScraper implements IJobAdScraper {
    /**
   * @description Function that accepts client form, and based on it starts the jobAd scraping.
   * Data available on CvLibrary in the scrape is (jobTitle, postedDate, salary, companyLocation, timeEngagement).
   ...
   */
    public async scrape(clientForm: ScrapeJobAdsForm): Promise<JobAdDTO[]> {
        const urlParams: AdScraperUrlParams = {
            jobTitle: clientForm.jobTitle.replace(Constants.WHITESPACE, Constants.MINUS_SIGN) + Constants.CV_LIBRARY_JOBLINK_SUFFIX,
            reqNofAds: clientForm.reqNOfAds
        }
        
        return await this.scrapeAds(urlParams, JobAdSource.CV_LIBRARY);
    }
}
